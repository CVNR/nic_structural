...
# Edits by Mark Vernon (mverno2@emory.edu)
# Changed output image naming scheme and added 0.5mm brain mask option
...
echo "* if option is -o script uses MNI152_T1_0.5mm_brain_mask_final.nii.gz for mask (default)"
...
echo "* outputs binarized brain-extraction mask, saved as:  <input_image>_brain-mask.nii.gz"
echo "* and full-intensity brain-extraction, saved as: <input_image>_brain.nii.gz"
echo ""
exit 1
}
[ "$1" = "" ] && howtouse
...
mask=MNI152_T1_0.5mm_brain_mask_final.nii.gz; #default MNI mask
...
if [[ ! -z $oopt ]]
then
mask=MNI152_T1_0.5mm_brain_mask_final.nii.gz; #use 0.5mm mask
fi
if [[ ! -z $topt ]]
then
...
fslmaths ${i}_step6.nii.gz -bin ${i}_brain-mask
...
fslmaths ${i} -mas ${i}_brain-mask ${i}_brain
